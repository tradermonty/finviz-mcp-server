...
VALID_PREFIXES = (
    'cap_', 'sh_', 'ta_', 'fa_', 'ind_',
    'earningsdate_', 'sec_', 'geo_', 'ah_',
)
...
class TestVolumeSurgeURL:
    def test_no_comma_concat(self, screener):
        ...
        assert not _has_no_comma_concat(tokens), f"Found token without valid prefix in {tokens}"

    def test_no_duplicates(self, screener):
        filters = screener._build_volume_surge_filters()
        params = screener._convert_filters_to_finviz(filters)
        tokens = _filter_tokens(params['f'])
        assert not _has_duplicates(tokens), f"Duplicate filter tokens: {_has_duplicates(tokens)}"

    def test_required_filters_present(self, screener):
        filters = screener._build_volume_surge_filters()
        params = screener._convert_filters_to_finviz(filters)
        f = params['f']
        for required in ('cap_smallover', 'sh_price_o10', 'ta_change_u2', 'sh_relvol_o1.5'):
...
class TestEarningsAfterhoursURL:
    ...
    def test_required_filters_present(self, screener):
        ...
        params = screener._convert_filters_to_finviz(filters)
        f = params['f']
        for required in ('cap_largeover', 'sh_price_o30', 'ah_change_u2', 'earningsdate_todayafter'):
            assert required in f, f"Missing required filter: {required}"
...
class TestEarningsTradingURL:
    ...
    def test_required_filters_present(self, screener):
        ...
        f = params['f']
        for required in (
            'cap_largeover', 'sh_price_o30', 'fa_netmargin_3to', 'fa_epsrev_ep',
            'sh_avgvol_o200', 'ta_change_u', 'ta_perf_0to-4w',
...
class TestEarningsPremarketURL:
    def test_required_filters_present(self, screener):
        ...
        params = screener._convert_filters_to_finviz(filters)
        f = params['f']
        for required in ('cap_largeover', 'sh_price_o30', 'sh_avgvol_o100', 'earningsdate_todaybefore'):
            assert required in f, f"Missing required filter: {required}"
