...
class TestE2EWithRealObjects:
    ...
    def test_stockdata_attribute_access_patterns(self):
        ...
        try:
            ...
            performance_info = [
                (
                    f"1W Performance: {stock.performance_1w:.2f}%"
                    if stock.performance_1w
                    else "1W Performance: N/A"
                ),
                (
                ...
                (
                    f"3M Performance: {stock.performance_3m:.2f}%"
                    if stock.performance_3m
                    else "3M Performance: N/A"
                ),
            ]
            ...
            earnings_info = [
                ...
                (
                    f"EPS QoQ Growth: {stock.eps_qoq_growth:.2f}%"
                    if stock.eps_qoq_growth
                    else "EPS QoQ Growth: N/A"
                ),
            ]
    ...
    def test_all_performance_attributes_exist(self):
        ...
        for attr in performance_attrs:
            ...
            if value is not None:
                assert isinstance(
                    value, (int, float)
                ), f"{attr} has invalid type: {type(value)}"
    ...
    def test_error_prone_attribute_combinations(self):
        ...
        try:
            price_text = (
                f"Price: ${partial_stock.price:.2f}"
                if partial_stock.price
                else "Price: N/A"
            )
            perf_1w_text = (
                f"1W Performance: {partial_stock.performance_1w:.2f}%"
                if partial_stock.performance_1w
                else "1W Performance: N/A"
            )
            perf_1m_text = (
                f"1M Performance: {partial_stock.performance_1m:.2f}%"
                if partial_stock.performance_1m
                else "1M Performance: N/A"
            )
            eps_text = (
                f"EPS Surprise: {partial_stock.eps_surprise:.2f}%"
                if partial_stock.eps_surprise
                else "EPS Surprise: N/A"
            )
