...
def analyze_volume_surge_stocks():
    ...
    try:
        ...
        # 価格変動率別分析
        price_change_ranges = {"10%以上": [], "5-10%": [], "2-5%": []}
        ...
        for i, stock in enumerate(results[:20]):  # 上位20銘柄を詳細分析
            ...
            sector_analysis[sector].append(
                {"ticker": ticker, "price_change": price_change, "volume": volume}
            )
        ...
        for performer in top_performers:
            print(
                f"{performer['rank']:2d}. {performer['ticker']:6s} | "
        ...
        for sector, stocks in sector_analysis.items():
            count = len(stocks)
            avg_change = (
                sum(s["price_change"] for s in stocks) / count if count > 0 else 0
            )
        ...
        for range_name, tickers in price_change_ranges.items():
            print(
                f"{range_name:10s}: {len(tickers):3d}銘柄 ({len(tickers)/len(results)*100:4.1f}%)"
            )
            if tickers:
        ...
        return {
            ...
            "price_change_distribution": {
                k: len(v) for k, v in price_change_ranges.items()
            },
            "market_cap_distribution": {
                k: len(v) for k, v in market_cap_ranges.items()
            },
            "top_sectors": sorted(
                sector_summary.items(), key=lambda x: x[1]["count"], reverse=True
            )[:5],
        }
