...
class ValidationResult:
    ...
    def get_summary(self) -> str:
        ...
        summary = (
            f"{len(self.valid_fields)} valid, {len(self.invalid_fields)} invalid fields"
        )
...
class FieldSearchEngine:
        ...
        for field_name, metadata in self.fields.items():
            ...
            if keyword_lower in field_name.lower():
                matches.append(
                    (
                        metadata,
                        self._calculate_relevance(keyword_lower, metadata, "name"),
                    )
                )
                continue
            ...
            if keyword_lower in metadata.display_name.lower():
                matches.append(
                    (
                        metadata,
                        self._calculate_relevance(keyword_lower, metadata, "display"),
                    )
                )
                continue
                ...
                matches.append(
                    (
                        ...
                        self._calculate_relevance(
                            keyword_lower, metadata, "description"
                        ),
                    )
                )
                continue
    ...
    def _calculate_relevance(
        self, keyword: str, metadata: FieldMetadata, match_type: str
    ) -> int:
    ...
    def validate(self, fields: List[str]) -> ValidationResult:
        ...
        if not fields:
            return ValidationResult(
                all_valid=True, valid_fields=[], invalid_fields=[], suggestions={}
            )
        ...
        for field in fields:
            ...
            if field in self.common_corrections:
                ...
                close_matches = difflib.get_close_matches(
                    field, self.valid_fields, n=3, cutoff=0.6
                )
